...
$(document).ready( async () => {
    try {
        ...
        const fetchPosts = await Promise.all(fetchBlog);
        
        // get user for each post
        const userPromises = fetchPosts.map( 
            fetchPosts => getObject(`${domain}/users/${fetchPosts.userId}`)
        );
        const users = await Promise.allSettled(userPromises);

        displayPosts(fetchPosts, users);
    } 
    catch(e) {
        alert(e.message)
    }
